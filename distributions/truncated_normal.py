...
class SquashedMeanTruncatedNormal(TruncatedNormal):
    """
    Truncated Normal distribution
    https://people.sc.fsu.edu/~jburkardt/presentations/truncated_normal.pdf
    """
    ...
    def __init__(self, loc, scale, a, b, eps=1e-8, validate_args=None):
        ...
        # squash the loc to be in (-1, 1), then spread that range to be (a, b)
        loc = (torch.tanh(loc) + 1) * (b - a) / 2 + a
        super().__init__(loc, scale, a, b, eps)
